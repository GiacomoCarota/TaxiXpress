...
const validateBookingData = (data) => {
  ...
  if (
    data.estimated_fare &&
    (isNaN(data.estimated_fare) || data.estimated_fare < 0)
  ) {
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const user = users[0];
    console.log(
      "📋 Ricerca utente completata:",
      user ? "Trovato" : "Non trovato"
    );
...
app.post("/api/bookings", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        "Errore interno del server durante il salvataggio della prenotazione",
    });
  }
});
...
app.get("/api/bookings/user/:userId", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ Trovate ${bookings.length} prenotazioni per utente ${userId}`
    );
...
app.put("/api/bookings/:bookingId/status", async (req, res) => {
  ...
  console.log("🔄 Aggiornamento status prenotazione:", {
    bookingId,
    status,
    driver_id,
  });
  ...
  try {
    ...
    console.log("✅ Status prenotazione aggiornato:", {
      bookingId,
      status,
      driver_id,
    });
...
app.delete("/api/bookings/:bookingId", async (req, res) => {
  ...
  console.log("🗑️ Richiesta cancellazione prenotazione:", {
    bookingId,
    user_id,
  });
...
app.put("/api/bookings/:bookingId/assign", async (req, res) => {
  ...
  console.log("👨‍💼 Assegnazione prenotazione a driver:", {
    bookingId,
    driver_id,
    driver_name,
  });
  ...
  try {
    ...
    console.log("✅ Prenotazione assegnata al driver:", {
      bookingId,
      driver_name,
    });
...
app.get("/api/bookings/driver/:driverId", async (req, res) => {
  ...
  try {
    ...
    FROM bookings b
    LEFT JOIN users u ON b.user_id = u.idu
...
`;
    ...
    console.log(
      `✅ Trovate ${driverBookings.length} prenotazioni per driver ${driverId}`
    );
...
app.put("/api/users/:userId", async (req, res) => {
  ...
  console.log("🔄 Aggiornamento utente:", {
    userId,
    firstName,
    lastName,
    email,
...
app.post("/api/promotions/validate", async (req, res) => {
  ...
  try {
    ...
    if (
      promotion.utilizzi_massimi &&
      promotion.utilizzi_correnti >= promotion.utilizzi_massimi
    ) {
      return res.status(400).json({
        success: false,
...
app.post("/api/promotions/:id/use", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ Promozione applicata: ${promoId}, utilizzi: ${updatedPromo[0].utilizzi_correnti}`
    );
